...
bool AppendToFile(const FilePath& filename, const char* data, int size) {
  ...
  if (fd < 0) {
    PLOG(ERROR) << "Unable to create file " << filename.value();
    return false;
  }

  // This call will either write all of the data or return false.
  if (!WriteFileDescriptor(fd, data, size)) {
    PLOG(ERROR) << "Error while writing to file " << filename.value();
    ret = false;
  }

  if (IGNORE_EINTR(close(fd)) < 0) {
    PLOG(ERROR) << "Error while closing file " << filename.value();
    return false;
  }
